...
async function loadConfig() {
  ...
  keySourceView();
  $("endpoint").value = data.endpoint.startsWith("https://")
    ? data.endpoint
    : "";
...
async function checker(older = false) {
  ...
  try {
    ...
    if (epoch !== generation || !token || code !== $("checker-room").value)
      return;
    traces = older
      ? [
          ...traces,
          ...data.traces.filter((t) => !traces.some((old) => old.id === t.id)),
        ]
      : data.traces;
...
async function selectMenu(name) {
  ...
  for (const id of ["ai", "checker", "history"])
