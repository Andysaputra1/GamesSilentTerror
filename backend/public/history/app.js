'use strict';
// Ambil elemen halaman riwayat berdasarkan ID.
const el = (id) => document.getElementById(id);
let token = '',
  after = 0;
// Kirim request riwayat dengan sesi yang hanya disimpan di memori halaman.
async function request(url, options = {}) {
  ...
  if (!response.ok)
    throw new Error(typeof data.detail === 'string' ? data.detail : 'Permintaan gagal.');
  return data;
}
// Muat halaman pesan berikutnya atau reset daftar ketika filter berubah.
  ...
  try {
    ...
    for (const row of data.messages) {
      ...
      for (const value of [
        ...
        `${row.room_code || '-'} / ${row.match_id || '-'} / ${row.round_number ?? '-'} / ${row.phase || '-'}`,
        `${row.sender_name} (${row.sender_kind})`,
        row.message,
      ]) {
    ...
    el('status').textContent = data.messages.length
...
el('logout').onclick = async () => {
  try {
    ...
    /* Clear local access even when the server is unavailable. */
  }
  token = '';
  after = 0;
  el('messages').replaceChildren();
  ...
  el('status').textContent = 'Keluar dari halaman riwayat.';
};
