...
def _cipher():
    ...
    try:
        ...
        raise PersistenceError("Kunci enkripsi panel belum siap. Jalankan setup_panel.py di server.") from error
...
def _decode_config(stored):
    ...
    if encrypted and source == "custom":
        try:
            override["openrouter_api_key"] = SecretStr(_cipher().decrypt(encrypted.encode()).decode())
        except (InvalidToken, UnicodeError) as error:
            raise PersistenceError("API key panel tidak dapat dibuka. Periksa PANEL_ENCRYPTION_KEY server.") from error
    return override
...
def save_panel_configuration(provider, endpoint, api_key=None, key_source=None):
    ...
    from sqlalchemy import text
    from services.persistence_service import PersistenceService
    stored = {"ai_provider": provider, "api_backend": "openrouter",
              "ollama_model": "qwen3:14b", "openrouter_model": "qwen/qwen3-14b",
              "ollama_base_url": endpoint}
    with ai_runtime.lock:
        def write(db):
            previous = db.execute(text("SELECT config FROM panel_configuration WHERE id=1 FOR UPDATE")).scalar()
            previous = json.loads(previous) if isinstance(previous, str) else (previous or {})
            source = key_source or ("custom" if api_key else previous.get("openrouter_key_source", "custom" if previous.get("openrouter_key_encrypted") else "default"))
            ...
            if provider == "api" and source == "custom" and not stored.get("openrouter_key_encrypted"):
