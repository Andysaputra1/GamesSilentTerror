...
existing = {line.split("=",1)[0]:line.split("=",1)[1] for line in content.splitlines() if "=" in line and not line.lstrip().startswith("#")}
def add(name, value):
    global content
...
if not existing.get("PANEL_PASSWORD_HASH", "").strip():
    ...
    if len(password) < 8 or password != getpass.getpass("Ulangi password: "):
        raise SystemExit("Password minimal 8 karakter dan konfirmasi harus cocok. Jalankan ulang script.")
    configure(path, password)
else:
    ensure_encryption(path)
print(".env.azure siap. Isi GOOGLE_CLIENT_ID dan opsional openrouter_default lewat nano .env.azure. Password database lama dipertahankan.")
