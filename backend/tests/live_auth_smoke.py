...
def main():
    ...
    try:
        with TestClient(app) as client:
            body = {'username':name, 'email':email, 'email_confirmation':email,
            ...
            assert client.post('/api/auth/login', json={'username':email, 'password':'wrong'}).status_code == 401
            ...
            print('PASS real MySQL: register, session, duplicate, wrong password, email login, logout/revocation')
    finally:
        with SessionLocal.begin() as db:
            # Identifikasi tepat username dan email buatan tes; bukan wildcard akun pengguna.
            row = db.execute(text('SELECT u.id FROM user_accounts u JOIN account_identities i ON i.user_id=u.id WHERE u.username=:name AND i.email=:email'), {'name':name,'email':email}).scalar()
            if row:
                db.execute(text('DELETE FROM user_accounts WHERE id=:id AND username=:name'), {'id':row,'name':name})
