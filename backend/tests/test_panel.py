...
class PanelTests(unittest.TestCase):
    ...
    def test_all_data_and_mutations_require_admin_session(self):
        for path in ['/api/panel/config','/api/panel/rooms','/api/panel/history','/api/panel/history.csv','/api/panel/checker/ABC123']:
            self.assertEqual(self.client.get(path).status_code,401)
        self.assertEqual(self.client.put('/api/panel/config',json={'provider':'api'}).status_code,401)
    ...
    def test_html_has_no_embedded_password_and_csp(self):
        ...
        self.assertEqual(self.client.get('/admin',follow_redirects=False).headers['location'],'/panel')
    ...
    def test_wrong_password_does_not_access_database(self):
        ...
        with patch.object(settings,'panel_password_hash',SecretStr(hashed)),patch('controller.api.panel.transaction') as db:
            self.assertEqual(self.client.post('/api/panel/login',json={'username':'administrator','password':'wrong'}).status_code,401)
    ...
    def test_tunnel_rejects_credentials_private_addresses_and_http(self):
        for url in ['http://example.com','https://name:pass@example.com','https://example.com/?key=secret']:
            with self.assertRaises(HTTPException): validate_endpoint(url)
        with patch('controller.api.panel.socket.getaddrinfo',return_value=[(2,1,6,'',('169.254.169.254',443))]):
            with self.assertRaises(HTTPException): validate_endpoint('https://metadata.test')

    def test_check_ai_error_is_not_reported_active(self):
        self.app.dependency_overrides[require_panel]=lambda:'fixture-token'
        with patch('controller.api.panel.api_reply',AsyncMock(side_effect=ValueError('secret must not leak'))):
    ...
    def test_empty_room_is_archived_and_db_failure_removes_live_room(self):
        ...
        self.app.dependency_overrides[require_authenticated_user]=lambda:SimpleNamespace(username='alice')
        rooms=RoomService()
        with patch('controller.api.rooms.room_service',rooms), patch('controller.api.rooms.PersistenceService') as factory:
...
class OpenRouterTests(unittest.IsolatedAsyncioTestCase):
    async def test_fixed_qwen_payload_and_no_secret_in_body(self):
        client=AsyncMock()
        client.post.return_value=httpx.Response(200,request=httpx.Request('POST','https://openrouter.ai'),json={'choices':[{'message':{'content':'OK'}}]})
        with patch('module.openrouter_client.httpx.AsyncClient') as factory:
            factory.return_value.__aenter__.return_value=client
            result=await generate_reply('hello',config=Settings(_env_file=None,openrouter_api_key='fixture-key'))
        self.assertEqual(result,'OK')
        self.assertEqual(client.post.call_args.kwargs['json']['model'],'qwen/qwen3-14b')
        self.assertEqual(client.post.call_args.kwargs['headers']['Authorization'],'Bearer fixture-key')
