...
class HistoryAccessTests(unittest.TestCase):
    ...
    def test_admin_pagination_and_no_store(self):
        self.app.dependency_overrides[require_authenticated_user] = lambda: SimpleNamespace(username='alice')
        with patch.object(settings, 'history_admin_usernames', 'alice'), patch('controller.api.history.PersistenceService._run', return_value=[{'id': 10}, {'id': 11}]):
            response = self.client.get('/api/history/messages?limit=1')
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.headers['cache-control'], 'no-store')
            self.assertEqual(response.json(), {'messages': [{'id': 10}], 'has_more': True, 'next_after_id': 10})
...
class ChatDurabilityTests(unittest.IsolatedAsyncioTestCase):
    async def run_chat(self, *, ai_fails=False, db_fails=False, bot_db_fails=False):
        ...
        with patch('realtime.socket_handlers.room_service', rooms), patch('realtime.socket_handlers.PersistenceService', return_value=persistence):
