...
class ActiveRoomTests(unittest.TestCase):
    ...
    def test_api_lookup_route_and_skip_validation(self):
        ...
        with patch("controller.api.rooms.room_service", self.rooms), TestClient(app) as client:
            self.assertEqual(
                client.get("/api/rooms/active").json()["active"]["code"], self.room.code
            )
            self.assertIsNone(
                client.get("/api/rooms/active", headers={"x-user": "outsider"}).json()["active"]
            )
            ...
            self.assertIsNone(
                client.get("/api/rooms/current", headers={"x-user": "outsider"}).json()["room"]
            )
            endpoint = "/api/rooms/" + self.room.code + "/skip-discussion"
            payload = {"match_id": self.room.match.id, "round_number": 1, "phase": "day"}
            self.assertEqual(
                client.post(endpoint, json={**payload, "match_id": "stale"}).status_code, 400
            )
            self.assertEqual(
                client.post(endpoint, json=payload, headers={"x-user": "outsider"}).status_code, 400
            )
            self.assertEqual(client.post(endpoint, json=payload).json()["game"]["phase"], "day")
            self.assertEqual(
                client.post(endpoint, json=payload, headers={"x-user": "bob"}).json()["game"][
                    "phase"
                ],
                "night",
            )
