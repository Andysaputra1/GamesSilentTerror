    ...
    try:
        with SessionLocal.begin() as database:
            for name in accounts:
                ids.append(create_account(database, username=name, display_name=name, password_hash=hash_password(password)))
        for name in accounts:
        ...
        states = {name:client.get(base + '/game').json()['game'] for name,client in clients.items()}
        ...
        def play(role, target_role, ability=None, expected=200):
            name = roles[role]
            view = state(name)
            result = clients[name].post(base+'/play', json={'match_id':view['id'], 'round_number':view['round'], 'phase':view['phase'], 'ability':ability, 'target':roles[target_role]})
        ...
        assert victim['me']['alive'] and not victim['me']['can_vote'] and not victim['me']['can_chat']
        play('civilian','hitman',expected=400)
        play('spy','hitman')
        ...
        print('PASS Hostage diam/tetap hidup, vote satu kali, eksekusi Hitman, WARGA MENANG. Room '+code, flush=True)
        ...
        with SessionLocal.begin() as database:
            for account_id in ids:
                database.execute(text('DELETE FROM user_accounts WHERE id = :id AND username LIKE :prefix'), {'id':account_id, 'prefix':prefix+'%'})
        print('CLEANUP: hanya akun QC yang dibuat tes dihapus; akun pengguna tidak diubah.', flush=True)
        if code:
