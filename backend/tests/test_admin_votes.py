...
class AdminTests(unittest.TestCase):
    ...
    def test_anonymous_and_nonadmin_denied_for_read_and_write(self):
        ...
        self.assertEqual(
            self.client.post(
                "/api/admin/provider", json={"provider": "api", "model": "8"}
            ).status_code,
            401,
        )
        self.app.dependency_overrides[require_authenticated_user] = lambda: SimpleNamespace(
    ...
    def test_provider_validate_check_availability_and_reset(self):
        self.admin()
        config = Settings(
            _env_file=None, ai_provider="api", OLLAMA_MODEL="8", OPENAI_API_KEY="fixture-secret"
        )
        with patch("services.ai_runtime_service.settings", config):
            with patch("controller.api.admin.model_available", AsyncMock(return_value=False)):
                self.assertEqual(
                    self.client.post(
                        "/api/admin/provider", json={"provider": "docker", "model": "14"}
                    ).status_code,
                    400,
                )
            ...
            with patch("controller.api.admin.model_available", AsyncMock(return_value=True)):
                ...
                self.assertEqual(
                    self.client.get("/api/admin/status").headers["cache-control"], "no-store"
                )
            self.assertEqual(
                self.client.post(
                    "/api/admin/provider", json={"provider": "other", "model": "14"}
                ).status_code,
                422,
            )
    ...
    def test_production_denies_admin_even_with_admin_account(self):
        self.admin()
        with patch(
            "controller.api.admin.settings", Settings(_env_file=None, app_environment="production")
        ):
